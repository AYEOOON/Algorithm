...
def self_Number(n):                 # 백준문제에 나온 d(n) 함수 
    number = int(n)                 # 33이 들어오면
    for i in list(str(n)):          # int형 변수 n을 string형으로 변환 후 리스트로 쪼개서 [3,3] 으로 나누고
        number += int(i)            # n의 각 자리수 i를 number에 저장된 숫자와 더해 다시 number에 저장, 즉 33 + 3 + 3
